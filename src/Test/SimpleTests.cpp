...
TEST_CASE("Yule process with no hybridization. [Yule][Analytical]") {
	...
	// read the tree file
	std::string newick_string = "((A:0.5,B:0.5):0.25,C:0.75):0.25;";

	// read the tree
	interface.readNewick(newick_string);
	double lnl = interface.computeLogLikelihood();

	REQUIRE(lnl == Approx(-1.1250000000).margin(1.e-7));
...
TEST_CASE("Yule process with asymmetrical hybridization. [Yule][Analytical]") {
	...
	// read the tree file
	std::string newick_string = "((A:0.5,C#A:0.00):0.25,(B:0.5)C#A:0.25):0.25;";

	// read the tree
	interface.readNewick(newick_string);
	double lnl = interface.computeLogLikelihood();

	REQUIRE(lnl == Approx(-3.9457322736).margin(1.e-7));
...
TEST_CASE("Yule process with symmetrical hybridization. [Yule][Analytical]") {
	...
	// read the tree file
	std::string newick_string = "((A:0.5,C#S1:0.00)D#S2:0.25,(B:0.5,D#S2:0.00)C#S1:0.25):0.25;";

	// read the tree
	interface.readNewick(newick_string);
	double lnl = interface.computeLogLikelihood();

	REQUIRE(lnl == Approx(-3.9082322736).margin(1.e-7));
...
TEST_CASE("Yule process with hybrid speciation. [Yule][Analytical]") {
	...
	// read the tree file
	std::string newick_string = "((A:0.5,(B:0.5)D#H:0.0):0.25,(C:0.5,D#H:0.0):0.25):0.25;";

	// read the tree
	interface.readNewick(newick_string);
	double lnl = interface.computeLogLikelihood();

	REQUIRE(lnl == Approx(-3.846517).margin(1.e-7));
...
TEST_CASE("Yule process with allopolyploid speciation. [Yule][Analytical]") {
	...
	// read the tree file
	std::string newick_string = "((A:0.5,(B:0.5)D#P:0.0):0.25,(C:0.5,D#P:0.0):0.25):0.25;";

	// read the tree
	interface.readNewick(newick_string);
	double lnl = interface.computeLogLikelihood();

	REQUIRE(lnl == Approx(-5.3509550779).margin(1.e-7));
...
TEST_CASE("Yule process with incomplete sampling. [Yule][Analytical]") {
	...
	REQUIRE(lnl == Approx(-2.1918377947).margin(1.e-7));
...
TEST_CASE("Birth-death process with incomplete sampling. [BD][Analytical]") {
	...
	REQUIRE(lnl == Approx(-2.3702210866).margin(1.e-7));
